...
// if value is there in local storage then updateUI will work
const localCity = localStorage.getItem('cityKey');
if(localCity){
  //get all data
  cityData(localCity)
  .then(data => updateUI(data))
...
// If you want to vanish the local storage data after browser close, You have to use session storage

// set time out trick to vanish the local storage data
setTimeout(() => {
  localStorage.clear();
